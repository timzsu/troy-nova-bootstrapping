...
void Polynomial::homomorphic_poly_evaluation(CKKSEvaluator *ckks, troy::Ciphertext &rtn, troy::Ciphertext &cipher) {
  ...
  if (deg == 1) {
    ...
    vector<troy::Ciphertext> baby(heap_k, troy::Ciphertext());
    ...
    vector<troy::Ciphertext> giant(heap_m, troy::Ciphertext());
    ...
    vector<troy::Ciphertext> cipherheap((1 << (heap_m + 1)) - 1, troy::Ciphertext());
