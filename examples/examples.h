...
inline void print_parameters(const troy::HeContext &context)
{
    ...
    */
    ...
    troy::utils::Array<troy::Modulus> coeff_modulus(coeff_modulus_device.size(), false);
    ...
    */
    if (context_data->parms().scheme() == troy::SchemeType::BFV || context_data->parms().scheme() == troy::SchemeType::BGV)
    {
        std::cout << "|   plain_modulus: " << context_data->parms().plain_modulus_host().value() << std::endl;
    }
...
*/
inline std::ostream &operator<<(std::ostream &stream, troy::ParmsID parms_id)
{
    /*
