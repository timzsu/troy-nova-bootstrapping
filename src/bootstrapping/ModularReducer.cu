...
void ModularReducer::double_angle_formula(troy::Ciphertext &cipher) {
...
void ModularReducer::double_angle_formula_scaled(troy::Ciphertext &cipher, double scale_coeff) {
  ckks->evaluator->square_inplace(cipher);
  ckks->evaluator->relinearize_inplace(cipher, ckks->relin_keys);
  ckks->evaluator->rescale_to_next_inplace(cipher);
  ckks->evaluator->double_inplace(cipher);
...
void ModularReducer::modular_reduction(troy::Ciphertext &rtn, troy::Ciphertext &cipher) {
  troy::Ciphertext tmp1, tmp2;
