...
Bootstrapper::Bootstrapper(
    ...
    long _inverse_deg,
    CKKSEvaluator *_ckks,
    troy::KeyGenerator *_keygen)
    : loge(_loge), logn(_logn), logNh(_logNh), L(_L), final_scale(_final_scale), boundary_K(_boundary_K), sin_cos_deg(_sin_cos_deg), scale_factor(_scale_factor), inverse_deg(_inverse_deg), ckks(_ckks), keygen(_keygen) {
  n = 1 << logn;
  Nh = 1 << logNh;
  mod_reducer =
...
void Bootstrapper::addBootKeys(troy::GaloisKeys &gal_keys) {
  ...
  keygen->create_galois_keys_from_steps(gal_steps_vector, false);
...
void Bootstrapper::addBootKeys_3(troy::GaloisKeys &gal_keys) {
  vector<int> gal_steps_vector;
  gal_steps_vector.push_back(0);
  for (int i = 0; i < logNh; i++) {
    gal_steps_vector.push_back((1 << i));
...
void Bootstrapper::addBootKeys_3_other_slots(troy::GaloisKeys &gal_keys, vector<long> &other_logn_vec) {
  vector<int> gal_steps_vector;
  gal_steps_vector.push_back(0);
  for (int i = 0; i < logNh; i++) {
    gal_steps_vector.push_back((1 << i));
...
void Bootstrapper::addBootKeys_3_other_slots_keys(troy::GaloisKeys &gal_keys, vector<long> &other_logn_vec, vector<int> &other_keys) {
  vector<int> gal_steps_vector;
  gal_steps_vector.push_back(0);
  for (int i = 0; i < logNh; i++) {
    gal_steps_vector.push_back((1 << i));
...
void Bootstrapper::addBootKeys_other_keys(troy::GaloisKeys &gal_keys, vector<int> &other_keys) {
  vector<int> gal_steps_vector;
  gal_steps_vector.push_back(0);
  for (int i = 0; i < logNh; i++) {
    gal_steps_vector.push_back((1 << i));
...
void Bootstrapper::addBootKeys_3_other_keys(troy::GaloisKeys &gal_keys, vector<int> &other_keys) {
  vector<int> gal_steps_vector;
  gal_steps_vector.push_back(0);
  for (int i = 0; i < logNh; i++) {
    gal_steps_vector.push_back((1 << i));
...
void Bootstrapper::addBootKeys_hoisting(troy::GaloisKeys &gal_keys) {
  vector<int> gal_steps_vector;
  gal_steps_vector.push_back(0);
  for (int i = 0; i < logNh; i++) {
    gal_steps_vector.push_back((1 << i));
...
void Bootstrapper::addBootKeys_one_depth(troy::GaloisKeys &gal_keys) {
  vector<int> gal_steps_vector;
  gal_steps_vector.push_back(0);
  for (int i = 0; i < logNh; i++) {
    gal_steps_vector.push_back((1 << i));
...
void Bootstrapper::addBootKeys_one_depth_more_depth(troy::GaloisKeys &gal_keys) {
  vector<int> gal_steps_vector;
  gal_steps_vector.push_back(0);
  for (int i = 0; i < logNh; i++) {
    gal_steps_vector.push_back((1 << i));
...
void Bootstrapper::subsum(double scale, troy::Ciphertext &cipher) {
  ...
  troy::Plaintext tmpplain;
  ckks->encoder.encode(1.0 / repeatcount, scale, tmpplain);
  ckks->evaluator->mod_switch_plain_to_inplace(tmpplain, cipher.parms_id());
...
void Bootstrapper::bsgs_linear_transform(
  ...
  vector<troy::Ciphertext> babyct(gs1, troy::Ciphertext());
  troy::Ciphertext giantct, tmpct;
  bool giantbool = false, tmpctbool = false;
  troy::Ciphertext tmptmpct;
  vector<complex<double>> rotatedcoeff;
  rotatedcoeff.reserve(Nh);

  troy::Plaintext tmpplain;
  troy::Ciphertext addtmp1, addtmp2;
...
void Bootstrapper::rotated_bsgs_linear_transform(
  ...
  vector<troy::Ciphertext> babyct(gs2, troy::Ciphertext());
  troy::Ciphertext giantct, tmpct;
  bool giantbool = false, tmpctbool = false;
  troy::Ciphertext tmptmpct;
  vector<complex<double>> rotatedcoeff;
...
void Bootstrapper::bsgs_linear_transform_hoisting(
  ...
  troy::Ciphertext *babyct = new troy::Ciphertext[gs1];
  troy::Ciphertext *giantct = 0, *tmpct = 0;
  troy::Ciphertext tmptmpct;
  vector<complex<double>> rotatedcoeff;
  rotatedcoeff.reserve(Nh);
...
void Bootstrapper::rotated_bsgs_linear_transform_hoisting(
  ...
  troy::Ciphertext *babyct = new troy::Ciphertext[gs2];
  troy::Ciphertext *giantct = 0, *tmpct = 0;
  troy::Ciphertext tmptmpct;
  vector<complex<double>> rotatedcoeff;
  rotatedcoeff.reserve(Nh);
...
void Bootstrapper::rotated_nobsgs_linear_transform(
    troy::Ciphertext &rtncipher, troy::Ciphertext &cipher, int totlen, int coeff_logn, vector<vector<complex<double>>> fftcoeff) {
  troy::Ciphertext *giantct = 0, *tmpct = 0;
...
void Bootstrapper::sfl_one_depth(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  int totlen1 = (1 << logn) - 1;
...
void Bootstrapper::sfl_full_one_depth(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  int totlen2 = (1 << logn) - 1;
  rotated_bsgs_linear_transform(rtncipher, cipher, totlen2, 1, logn, fftcoeff2[slot_index]);
}

void Bootstrapper::sflinv_one_depth(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  int totlen1 = (1 << logn) - 1;
  rotated_bsgs_linear_transform(rtncipher, cipher, totlen1, 1, logn, invfftcoeff1[slot_index]);
}

void Bootstrapper::sflinv_one_depth_more_depth(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  int totlen1 = (1 << logn) - 1;
  rotated_nobsgs_linear_transform(rtncipher, cipher, totlen1, logn, invfftcoeff1[slot_index]);
}
void Bootstrapper::sfl(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  int split_point = floor(logn / 2.0);
  int totlen1 = (1 << split_point) - 1;
  int totlen2 = (1 << (logn - split_point)) - 1;

  troy::Ciphertext tmpct;
  bsgs_linear_transform(tmpct, cipher, totlen1, 1, logn + 1, fftcoeff1[slot_index]);
  ckks->evaluator->rescale_to_next_inplace(tmpct);

  const auto &modulus = ckks->context->first_context_data()->get()->parms().coeff_modulus();
  auto curr_level = ckks->context->get_context_data(tmpct.parms_id())->get()->chain_index();
...
void Bootstrapper::sfl_full(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  int split_point = floor(logn / 2.0);
  int totlen1 = (1 << split_point) - 1;
  int totlen2 = (1 << (logn - split_point)) - 1;
...
void Bootstrapper::sflinv(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
...
void Bootstrapper::sflinv_full(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  int split_point = ceil(logn / 2.0);
  int totlen1 = (1 << split_point) - 1;
  int totlen2 = (1 << (logn - split_point)) - 1;
...
void Bootstrapper::sfl_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {  // not yet
  ...
  const auto modulus = ckks->context->first_context_data()->get()->parms().coeff_modulus();
  auto curr_level = ckks->context->get_context_data(tmpct2.parms_id())->get()->chain_index();
...
void Bootstrapper::sfl_full_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  int div_part3 = floor(logn / 3.0);
  int div_part2 = floor((logn - div_part3) / 2.0);
  int div_part1 = logn - div_part3 - div_part2;
...
void Bootstrapper::sfl_half_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {  // not yet
  int div_part3 = floor(logn / 3.0);
  int div_part2 = floor((logn - div_part3) / 2.0);
  int div_part1 = logn - div_part3 - div_part2;
...
void Bootstrapper::sfl_full_half_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  int div_part3 = floor(logn / 3.0);
  int div_part2 = floor((logn - div_part3) / 2.0);
  int div_part1 = logn - div_part3 - div_part2;
...
void Bootstrapper::sflinv_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {  // not yet
  int div_part1 = floor(logn / 3.0);
  int div_part2 = floor((logn - div_part1) / 2.0);
  int div_part3 = logn - div_part1 - div_part2;
...
void Bootstrapper::sflinv_full_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  int div_part1 = floor(logn / 3.0);
  int div_part2 = floor((logn - div_part1) / 2.0);
  int div_part3 = logn - div_part1 - div_part2;
...
void Bootstrapper::sfl_hoisting(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  int split_point = floor(logn / 2.0);
  int totlen1 = (1 << split_point) - 1;
  int totlen2 = (1 << (logn - split_point)) - 1;
...
void Bootstrapper::sfl_full_hoisting(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  int split_point = floor(logn / 2.0);
  int totlen1 = (1 << split_point) - 1;
  int totlen2 = (1 << (logn - split_point)) - 1;
...
void Bootstrapper::sflinv_hoisting(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  int split_point = ceil(logn / 2.0);
  int totlen1 = (1 << split_point) - 1;
  int totlen2 = (1 << (logn - split_point)) - 1;
...
void Bootstrapper::sflinv_full_hoisting(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  int split_point = ceil(logn / 2.0);
  int totlen1 = (1 << split_point) - 1;
  int totlen2 = (1 << (logn - split_point)) - 1;
...
void Bootstrapper::coefftoslot(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3;
...
void Bootstrapper::slottocoeff(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3;
  sfl(tmpct1, cipher);
...
void Bootstrapper::coefftoslot_full(troy::Ciphertext &rtncipher1, troy::Ciphertext &rtncipher2, troy::Ciphertext &cipher) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3, tmpct4, tmpct5;
  ...
  ckks->evaluator->mod_switch_plain_to_inplace(tmpplain, tmpct1.parms_id());
...
void Bootstrapper::slottocoeff_full(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher1, troy::Ciphertext &cipher2) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3;
  complex<double> iunit(0.0, 1.0);
  ...
  ckks->evaluator->mod_switch_plain_to_inplace(tmpplain, cipher2.parms_id());
...
// NOTE:
void Bootstrapper::coefftoslot_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3;
...
// NOTE:
void Bootstrapper::slottocoeff_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3;
  sfl_3(tmpct1, cipher);
  ckks->evaluator->rotate_vector(tmpct1, n, ckks->galois_keys, tmpct2);
  ckks->evaluator->add_reduced_error(tmpct1, tmpct2, rtncipher);
...
void Bootstrapper::slottocoeff_half_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3;
...
void Bootstrapper::coefftoslot_full_3(troy::Ciphertext &rtncipher1, troy::Ciphertext &rtncipher2, troy::Ciphertext &cipher) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3, tmpct4, tmpct5;
...
void Bootstrapper::slottocoeff_full_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher1, troy::Ciphertext &cipher2) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3;
  complex<double> iunit(0.0, 1.0);
...
void Bootstrapper::slottocoeff_full_half_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher1, troy::Ciphertext &cipher2) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3;
  complex<double> iunit(0.0, 1.0);
...
void Bootstrapper::coefftoslot_hoisting(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3;
...
void Bootstrapper::slottocoeff_hoisting(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3;
...
void Bootstrapper::coefftoslot_full_hoisting(troy::Ciphertext &rtncipher1, troy::Ciphertext &rtncipher2, troy::Ciphertext &cipher) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3, tmpct4, tmpct5;
...
void Bootstrapper::slottocoeff_full_hoisting(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher1, troy::Ciphertext &cipher2) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3;
  complex<double> iunit(0.0, 1.0);
...
void Bootstrapper::coefftoslot_one_depth(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3;
...
void Bootstrapper::slottocoeff_one_depth(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3;
  complex<double> iunit(0.0, 1.0);
...
void Bootstrapper::coefftoslot_full_one_depth(troy::Ciphertext &rtncipher1, troy::Ciphertext &rtncipher2, troy::Ciphertext &cipher) {}

void Bootstrapper::slottocoeff_full_one_depth(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher1, troy::Ciphertext &cipher2) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3;
  complex<double> iunit(0.0, 1.0);
...
void Bootstrapper::coefftoslot_full_mul_first(troy::Ciphertext &rtncipher1, troy::Ciphertext &rtncipher2, troy::Ciphertext &cipher) {
  troy::Ciphertext tmpct1, tmpct2, tmpct3, tmpct4, tmpct5;
...
void Bootstrapper::modraise_inplace(troy::Ciphertext &cipher) {
  if (cipher.polynomial_count() != 2) {
    throw invalid_argument("troy::Ciphertexts of size 2 are supported only!");
  }

  if (cipher.coeff_modulus_size() != 1) {
    throw invalid_argument("troy::Ciphertexts in the lowest level are supported only!");
  }
  ...
  const auto &modulus = context_data->get()->parms().coeff_modulus();
  ...
  cipher.resize(ckks->context, context_data->get()->parms_id(), 2);

  auto ciphertext_size = cipher.polynomial_count();
  troy::Ciphertext cipher_copy = cipher;

  uint64_t gridDimGlb = N / bootstrap::blockDimGlb.x;
  for (size_t poly_idx = 0; poly_idx < ciphertext_size; poly_idx++) {
    const auto rns_poly_src = cipher_copy.poly(poly_idx).raw_pointer();
    const auto rns_poly_dest = cipher.poly(poly_idx).raw_pointer();

    for (size_t j = 0; j < mod_count; j++) {
      const auto poly_dest = rns_poly_dest + j * N;
      kernel_modraise_inplace<<<gridDimGlb, bootstrap::blockDimGlb, 0>>>(
...
void Bootstrapper::bootstrap_sparse(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  ...
  troy::Ciphertext modrtn;
  mod_reducer->modular_reduction(modrtn, rtn);
  std::cout << "mod end" << endl;

  if (logn == 0) {
    const auto modulus = ckks->context->first_context_data()->get()->parms().coeff_modulus();
    auto curr_level = ckks->context->get_context_data(modrtn.parms_id())->get()->chain_index();
...
void Bootstrapper::bootstrap_full(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  // // for debugging
  ...
  std::cout << "Modular reduction..." << endl;
  troy::Ciphertext modrtn1, modrtn2;
...
void Bootstrapper::bootstrap_sparse_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  // ModRaise
  ...
  timer.stop();
  std::cout << timer.duration<std::chrono::milliseconds>() << "ms\n";
...
void Bootstrapper::bootstrap_full_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  std::cout << "Modulus Raising..." << endl;
  modraise_inplace(cipher);
...
void Bootstrapper::bootstrap_sparse_real_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  std::cout << "Modulus Raising..." << endl;
  modraise_inplace(cipher);
...
void Bootstrapper::bootstrap_full_real_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  std::cout << "Modulus Raising..." << endl;
  modraise_inplace(cipher);
...
void Bootstrapper::bootstrap_sparse_hoisting(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  std::cout << "Modulus Raising..." << endl;
  modraise_inplace(cipher);
...
void Bootstrapper::bootstrap_full_hoisting(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  std::cout << "Modulus Raising..." << endl;
  modraise_inplace(cipher);
...
void Bootstrapper::bootstrap_one_depth(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  std::cout << "Modulus Raising..." << endl;
  modraise_inplace(cipher);
...
void Bootstrapper::bootstrap_more_depth(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  std::cout << "Modulus Raising..." << endl;
  modraise_inplace(cipher);
...
void Bootstrapper::bootstrap(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
...
void Bootstrapper::bootstrap_inplace(troy::Ciphertext &cipher) {
  troy::Ciphertext rtncipher;
  bootstrap(rtncipher, cipher);
  cipher = rtncipher;
}

void Bootstrapper::bootstrap_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  if (ckks->context->get_context_data(rtncipher.parms_id())->get()->chain_index()) {
...
void Bootstrapper::bootstrap_inplace_3(troy::Ciphertext &cipher) {
  troy::Ciphertext rtncipher;
  bootstrap_3(rtncipher, cipher);
  cipher = rtncipher;
}

void Bootstrapper::bootstrap_real_3(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  initial_scale = cipher.scale();
  if (logn == logNh)
...
void Bootstrapper::bootstrap_inplace_real_3(troy::Ciphertext &cipher) {
  troy::Ciphertext rtncipher;
  bootstrap_real_3(rtncipher, cipher);
  cipher = rtncipher;
}

void Bootstrapper::bootstrap_hoisting(troy::Ciphertext &rtncipher, troy::Ciphertext &cipher) {
  if (logn == logNh)
...
void Bootstrapper::bootstrap_inplace_hoisting(troy::Ciphertext &cipher) {
  troy::Ciphertext rtncipher;
